...
// Requires the Discourse Gamification plugin, and an admin-configured
// "weekly" leaderboard for the title to be accurate -- the period comes
// from server-side leaderboard config, not from this block's args.
@block("theme:ctguns:leaderboard", {
    ...
    <AsyncContent @asyncData={{this.fetchLeaderboard}}>
      ...
      <:content as |data|>
        <div class="block-leaderboard__layout">
          {{#if @title}}
            <h2 class="block-leaderboard__title">{{i18n (themePrefix @title)}}</h2>
          {{/if}}

          <div class="block-leaderboard__list">
            {{#if data.currentUserNotInTop}}
              <div class="block-leaderboard__row --self">
                <span class="block-leaderboard__rank">{{data.personal.position}}</span>
                <span class="block-leaderboard__name">{{i18n "gamification.you"}}</span>
                <span class="block-leaderboard__score">{{number data.personal.user.total_score}}</span>
              </div>
            {{/if}}

            {{#each data.users as |rank|}}
              <div class="block-leaderboard__row {{if rank.isCurrentUser '--highlight'}}">
                <div class="block-leaderboard__user" data-user-card={{rank.username}}>
                ...
                <span class="block-leaderboard__score">{{number rank.total_score}}</span>
              </div>
            {{/each}}
          </div>
